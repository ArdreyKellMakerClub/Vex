...
#pragma config(Motor,  port1,           bot0,          tmotorVex393HighSpeed_HBridge, openLoop, reversed)
...
static const int LIFT_MIN = 2430;
...
task kill(){
	while(true){
		int anaCh1 = vexRT[Ch2];
		int anaCh2 = vexRT[Ch3];

		if(anaCh1 == 127 && anaCh2 == -127){
			stopTask(liftcontrol);
		} else if(anaCh1 == -127 && anaCh2 == 127){
			startTask(liftcontrol);
		}
